...
/*
  Fonction qui retourne la question en fonction de l'id
  Entrée : -
  Sortie : question: object
*/
exports.getQuestionFromID = function(id){
  return tableauQuestions[id-1];
};

/*
  Fonction qui retourne toutes les questions du tableau le nombre de question
  Entrée : tableauidDomaine: array of id domaine
  Sortie : nombre d'éléments du tableau de question:integer
*/
exports.getNumQuestions = function(tableauidDomaine){
  return this.getQuestionsFromDomaine(tableauidDomaine).length;
};

/*
  Fonction qui retourne une question aléatoire en fonction du domaine et des _questionPasse, si aucun argument alors retourne une question random entre 1 et 10
  Entrée : _domaine: array of ids, _questionPasse: array of ids
  Sortie : question aléatoire
*/
exports.getRandomQuestion = function(_domaine, _questionPasse){
  
  // je vérifie sur quel domaine je veux une question aléatoire
  if(_domaine && _questionPasse){
    var mesQuestions = this.getQuestionsFromDomaine(_domaine);
    do{
      maQuestion = Math.floor(Math.random() * mesQuestions.length);
    }while(_questionPasse.indexOf(mesQuestions[maQuestion].id)!=-1);
    console.log(mesQuestions[maQuestion]);
    return mesQuestions[maQuestion];
  }
  else{
    // question aléatoire entre 1 et 10
    maQuestion=Math.floor((Math.random() * 10) + 1);
    return this.getQuestionFromID(maQuestion);
  }
};